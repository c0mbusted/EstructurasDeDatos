...
int esABB(Nodo* root);
int sumaElementos(Nodo* root, int clave);
int esHoja(Nodo *n);
...
{ 
    ...
    { 
        ...
        return esABB(root->right); 
    } 
  
    return 1; 
} 

//fucion sumar elementos 

int sumaElementos(Nodo* root, int clave) 
{ 
    if (root == NULL) 
        return 0; 
    return (root->clave + addBT(root->left) + addBT(root->right)); 
} 